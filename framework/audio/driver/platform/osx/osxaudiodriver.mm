...
{
    auto* data = reinterpret_cast<OSXAudioDriver::Data*>(inClientData);
    if (data->stopPending) {
...
std::vector<ChannelBufferDetails> getChannelBufferDetails(const OSXAudioDeviceID& deviceId,
                                                          std::function<void(std::string, OSStatus)> onError)
{
...
bool OSXAudioDriver::open(const Spec& spec, Spec* activeSpec)
{
    ...
    OSStatus result = AudioObjectGetPropertyData(*deviceId, &bufferSizeRangeAddress, 0, 0, &bufferSizeRangeSize, &bufferSizeRange);
    if (result != noErr) {
    ...
    Float64 sampleRate = closestAvailableSampleRate(*deviceId, spec.output.sampleRate, &logError)
    ...
    if (outputStreamInfos.size()
        < (unsigned int)spec.output.audioChannelCount) {
        logError("Not enough channels are available with current device", noErr);
        return false;
    }
    if (spec.output.audioChannelCount == 2) {
        filterAndReorderStereoHardwareChannels(*deviceId, outputStreamInfos);
    }
    if (outputStreamInfos.size() < (unsigned int)spec.output.audioChannelCount) {
    ...
    if (outputStreamInfos.size()
        ...
        outputStreamInfos.resize(spec.output.audioChannelCount);
    }
    m_data->canBeDirectlyMapped = canBeDirectlyMapped(outputStreamInfos,
                                                      spec.output.audioChannelCount);
    m_data->format = spec;
    m_data->format.output.sampleRate = sampleRate;
    m_data->format.output.samplesPerChannel = bufferSizeOut;
    m_data->format.deviceId = QString::number(*deviceId).toStdString();
    m_data->channelBufferOutputDetails = std::move(outputStreamInfos);
    m_data->outBuffer.resize(bufferSizeOut
                             * spec.output.audioChannelCount);
    m_data->deviceId = *deviceId;

    result = AudioDeviceCreateIOProcID(*deviceId, coreAudioIOProc, m_data.get(),
                                       &m_data->procId);
    if (result != noErr) {
        m_data->clear();
...
static std::optional<OSXAudioDeviceID> defaultDeviceId()
{
...
std::optional<int> muse::audio::OSXAudioDriver::getAudioDeviceId(
    const AudioDeviceID& deviceId) const
{
    if (deviceId.empty() || deviceId == DEFAULT_DEVICE_ID) {
        return defaultDeviceId(); //default device used
    }
